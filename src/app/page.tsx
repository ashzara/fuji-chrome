...
export default function Home() {
    ...
    async (files: File[]) => {
      ...
      try {
        ...
        for (let i = 0; i < files.length; i++) {
          setProgressLabel(files.length > 1 ? `Photo ${i + 1} of ${files.length}…` : "Processing…");
  ...
  const handleReset = () => {
    results.forEach((r) => { URL.revokeObjectURL(r.beforeUrl); URL.revokeObjectURL(r.afterUrl); });
    setResults([]); setActiveModal(null);
    setAppState("idle"); setErrorMsg(""); setProgress(0);
  };
    ...
    <main className="min-h-screen flex flex-col items-center px-4 py-12 gap-10">
      ...
      {appState === "processing" && (
        <div className="flex flex-col items-center justify-center gap-4 w-full max-w-lg mx-auto rounded-2xl border-2 border-dashed border-[#2a2a2a] py-16 px-8">
          ...
          <div className="w-48 h-1 bg-[#222] rounded-full overflow-hidden">
            <div className="h-full bg-[#c8a882] rounded-full transition-all duration-150" style={{ width: `${progress}%` }} />
          </div>
      ...
      {appState === "error" && (
        <div className="w-full max-w-lg rounded-xl bg-red-950/40 border border-red-800/40 px-4 py-3 text-sm text-red-300 flex items-center justify-between">
          <span>{errorMsg}</span>
          <button onClick={handleReset} className="ml-4 underline text-red-400 hover:text-red-300 shrink-0">Try again</button>
        </div>
      )}
      ...
      {appState === "done" && results.length > 0 && (
        <div className="w-full max-w-4xl flex flex-col gap-6">
          <div className="flex items-center justify-between">
            ...
            {results.length > 1 && (
              <button onClick={handleDownloadAll} className="flex items-center gap-2 bg-[#c8a882] hover:bg-[#d4b896] text-black font-medium text-sm px-5 py-2.5 rounded-lg transition-colors">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {results.map((r) => (
              <div key={r.id} className="rounded-2xl overflow-hidden border border-[#2a2a2a] bg-[#111] cursor-pointer hover:border-[#444] transition-colors group" onClick={() => setActiveModal(r)}>
                <div className="relative aspect-square overflow-hidden">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={r.afterUrl} alt={r.name} className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center">
                    <span className="text-white text-xs font-medium opacity-0 group-hover:opacity-100 transition-opacity bg-black/60 px-3 py-1 rounded-full">Compare</span>
                  </div>
                </div>
      ...
      {activeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 p-4" onClick={() => setActiveModal(null)}>

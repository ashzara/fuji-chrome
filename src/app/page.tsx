...
export default function Home() {
  ...
  const [hasCustomLUT, setHasCustomLUT] = useState(false);

  const lutRef = useRef<Float32Array | null>(null);

  // Load LUT on mount — custom if saved, otherwise built-in Classic Chrome
  useEffect(() => {
    const saved = loadSavedLUT();
    if (saved) {
      lutRef.current = saved;
      setHasCustomLUT(true);
    } else {
      lutRef.current = createClassicChromeLUT(LUT_SIZE);
    }
  }, []);
  ...
  const handleLUTReady = useCallback((lut: Float32Array) => {
    lutRef.current = lut;
    saveLUT(lut);
    setHasCustomLUT(true);
  }, []);
  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      try {
        // 1. Decode image (respects EXIF rotation)
        const imageData = await fileToImageData(file);

        // 2. Normalize exposure in-place (same range every time)
        const pixels = new Uint8ClampedArray(imageData.data);
        normalizeExposure(pixels);

        // 3. Apply LUT with trilinear interpolation (chunked, non-blocking)
        const result = await applyLUT(pixels, getActiveLUT(), LUT_SIZE, setProgress);

        // 4. Export as JPEG blob
        const resultData = new ImageData(result, imageData.width, imageData.height);
  ...
  return (
    <main className="min-h-screen flex flex-col items-center px-4 py-12 gap-10">
      ...
      <header className="text-center">
        <h1 className="text-2xl font-semibold tracking-wide text-[#e8e8e8]">
          Fuji Chrome
        </h1>
      ...
      {/* Error */}
      {state === "error" && (
        <div className="w-full max-w-lg rounded-xl bg-red-950/40 border border-red-800/40 px-4 py-3 text-sm text-red-300">
          {errorMsg}
        </div>
      )}
      ...
      <div className="w-full max-w-3xl">
        <TrainingPanel
          onLUTReady={handleLUTReady}
          hasCustomLUT={hasCustomLUT}
